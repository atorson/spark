...
object HitsRank extends Logging {
  ...
  // static keys to extra algorithmic output
  /**
   * static key to access the actual # of iterations from extra algorithmic output
   * @usecase runWithExtraInfo()
   */
  val AlgIterationsNumber: String = "AlgIterationsNumber"

  /**
   * Executes a given number of HITS algorithm iterations unless convergence tolerance is achieved first.
   * <P>
   ...
   * used for iterative approximate computation of the dominant eigen-vector(as in Principal Component Analysis).
   ...
   * This technique requires caching of the powers of matrix '''A''' which is likely to be memory- and/or
   * communication-cost prohibitive for large graphs.  This HITS algorithm implementation is lean on caching
   * (only caches the previous and the current iteration's (Hub, Authority) ranks and only for the duration of the iteration)
   ...
   * @param convTolerance convergence tolerance(as measured by approximate Mean-Squared-Error, default is 0.0)
  ...
  def run[VD: ClassTag, ED: ClassTag](graph: Graph[VD, ED],
      initValue: RDD[(VertexId, (Double, Double))] = SparkContext.getOrCreate().emptyRDD[(VertexId, (Double, Double))],
      numIter: Int = 1, convTolerance: Double = 0.0): Graph[(Double, Double), ED] = {
   ...
   */
    def runWithExtraReturnInfo[VD: ClassTag, ED: ClassTag](graph: Graph[VD, ED],
        initValue: RDD[(VertexId, (Double, Double))] = SparkContext.getOrCreate().emptyRDD[(VertexId, (Double, Double))],
        numIter: Int = 1, convTolerance: Double = 0.0): (Graph[(Double, Double), ED], Map[String, Any]) = {
    ...
    var hitsGraph: Graph[(Double, Double), ED] = graph
      .outerJoinVertices[(Double,Double),(Double,Double)](initValue)(
    ...
    var extraAlgOutput = Map[String, Any]()
    ...
    for (i <- 0 until numIter if !convergence) {
      ...
      hitsGraph = hitsGraph.outerJoinVertices[Double, (Double,Double)](normalizeRanksUsingL2Norm(authRanks))(
      ...
      // such nodes do not affect and are not needed for the preceding normalization calculation
      hitsGraph = hitsGraph.outerJoinVertices[Double, (Double,Double)](normalizeRanksUsingL2Norm(hubRanks))(
      ...
      lazy val mseProxy = computeHitsMSEProxy(prev.vertices, hitsGraph.vertices)
      ...
      extraAlgOutput += (AlgIterationsNumber -> (i+1))
    }
  ...
  /**
   * Computes the L2 norm of the difference between previous and last HITS iteration rank values
   ...
   */
  def computeHitsMSEProxy(prev: VertexRDD[(Double,Double)], next: VertexRDD[(Double,Double)]): Double = {

    val errors = prev.innerJoin[(Double, Double), (Double, Double)](next)((vid, x, y) => (x._1 - y._1, x._2 - y._2))

    val error = errors.map[(Double, Double)](x => (x._2._1 * x._2._1, x._2._2 * x._2._2)).reduce(
      (x, y) => (x._1 + y._1, x._2 + y._2))

    error match {
      case (x: Double, y: Double) => math.sqrt(x + y)
      case _ => 0.0
    }
  }
  ...
  private def normalizeRanksUsingL2Norm(ranks: VertexRDD[Double]): VertexRDD[Double] = {
    ...
    val norm: Double = math.sqrt(ranks.map[Double]( x => (x._2 * x._2)).reduce(_+_))
